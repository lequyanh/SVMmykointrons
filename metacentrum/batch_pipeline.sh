...
while read fungi; do
  ...
  qsub -l walltime=24:0:0 -l select=1:ncpus=16:mem=4gb:scratch_local=2gb -v assembly_filepath="${assembly}",splice_site_donor_model=${dmodel},splice_site_acceptor_model=${amodel},intron_model=${imodel},phylum=${phylum},intron_source="${introns}",fungi_name="${fungi}" pipeline_perun.sh
